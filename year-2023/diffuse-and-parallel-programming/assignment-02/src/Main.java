...
class Order implements OrderInterface {
    ...
    public int order(Location location) {
        ...
        System.out.println("Created a order with ID " + lastOrderID + " to visit [" + location.col() + ", "
                + location.row() + "]");
    ...
    public void printLocationsOrdered() {
        ...
        for (Map.Entry<Location, Integer> entry : locationsOrdered.entrySet()) {
            System.out.println(
                    "[" + entry.getKey().col() + ", " + entry.getKey().row() + "] -> " + entry.getValue());
        }
    }
}
...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 18; j++) {
                if (values[i][j] == 'W') labirynth[i][j] = LocationType.WALL;
                else if (values[i][j] == 'P') labirynth[i][j] = LocationType.PASSAGE;
                else if (values[i][j] == 'E') labirynth[i][j] = LocationType.EXIT;
            }
        }
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 18; j++) {
                if (labirynth[i][j] == LocationType.WALL) System.out.print("W ");
                else if (labirynth[i][j] == LocationType.PASSAGE) System.out.print("P ");
                else if (labirynth[i][j] == LocationType.EXIT) System.out.print("E ");
            }
            System.out.println();
        }
        System.out.println();
